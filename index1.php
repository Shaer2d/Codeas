...
if(!isset($_SESSION['cart_item'])){
    $username=$_SESSION['username'];
    $select_query="SELECT favourites FROM users WHERE username = '$username'";
    $cart_result=$db_handle->runQuery($select_query);
    if($cart_result){
        $_SESSION['cart_item']=unserialize($cart_result[0]['favourites']);
    }
    else{
...
<div id="product-grid">
    ...
    <div class="price-filter">
        <form method="post" action="">
            ...
            <input id="price-slider" type="range" name="price_range" min="3000" max="13700000" step="1000" value="<?php echo isset($_POST['price_range']) ? $_POST['price_range'] : '13700000'; ?>">
    ...
    <div class="sort-by">
        <form method="post" action="index.php">
            ...
            <select name="sort_option" id="sort-by">
                <option value="ASC"<?php if(isset($_POST['sort_option']) && $_POST['sort_option'] == 'ASC') echo ' selected'; ?>>Low to High</option>
                <option value="DESC"<?php if(isset($_POST['sort_option']) && $_POST['sort_option'] == 'DESC') echo ' selected'; ?>>High to Low</option>
            </select>
    ...
    <?php
    // Get products from database and apply filters
    ...
    if(isset($_SESSION['price_range'])) {
        ...
        $filtered_array = array_filter($product_array, function($product) use($price_range) {
            return $product['price'] <= $price_range;
        });
        $product_array = array_values($filtered_array);
    }
    if(isset($_POST['sort_submit'])) {
    ...
    if(isset($_SESSION['sort_option'])) {
        ...
        usort($product_array, function($a, $b) use($sort_option) {
            if($sort_option == 'ASC') {
...
<div id="product-grid">
    <?php
    $query = "SELECT * FROM tblproduct WHERE price <= " . $_SESSION['price_range'];
    $sort = " ORDER BY price " . $_SESSION['sort_option'];
    $query .= $sort;
